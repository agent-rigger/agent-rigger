...
Publishing an MCP server has [its own page](/authoring/mcp-servers/), and so does declaring a
[tool](/authoring/tools/) dependency: its presence check works today, but its install from the
