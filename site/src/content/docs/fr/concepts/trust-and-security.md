...
![Les portes de confiance que le contenu récupéré franchit avant toute écriture, dans l'ordre d'exécution : provenance (sha du HEAD re-vérifié contre le sha résolu, refus en exit 2), scan (gitleaks et trivy, finding bloquant en exit 1, warn-only quand aucun scanner n'est installé, ou avertissement nommant l'outil manquant quand un seul scanner est installé et ne trouve rien), confirmation du plan, et consent par commande enregistré dans un ledger — avec --force qui ne couvre que la porte de scan, jamais la porte de provenance.](../../../../assets/diagrams/trust-gates.svg)
